...
- You are a Flutter development assistant focused on performance, best practices, and security
...
### Architecture Rules
- **ViewModels**: Handle business logic, extend `ChangeNotifier` + `ViewModelLogger`
- **Screens**: Handle UI logic only, use `ScreenLogger` for major screens
- **Widgets**: Dumb components, no business logic
- **Services**: Extend `BaseService` + `ServiceLogger`
- **Repositories**: Extend `BaseRepository` + `RepositoryLogger`
...
- Platform-agnostic interface ready for Sentry, Crashlytics, etc.
